...
const SpecificBeneficiary = () => {
  ...
  const [graduateForm, setGraduateForm] = useState({ reason: '', file: null });
  ...
  // Separate effect for fetching documents after beneficiary data is loaded
  useEffect(() => {
    if (!beneficiary) return;

    const fetchBeneficiaryDocuments = async () => {
      try {
        // Fetch beneficiary records (graduation/termination documents)
        const recordsResponse = await fetch(`http://localhost:5000/api/beneficiary-records/beneficiary/${id}`);
        let documents = [];
        
        if (recordsResponse.ok) {
          const recordsData = await recordsResponse.json();
          documents = recordsData.records || [];
        }

        // Add support letter if it exists
        if (beneficiary?.support_letter_url) {
          documents.push({
            id: `support-letter-${id}`,
            type: 'support_letter',
            title: 'Support Letter',
            file_url: `/uploads/${beneficiary.support_letter_url}`,
            created_at: beneficiary.created_at,
            isSystemDocument: true
          });
        }

        // Add bank book photo if it exists
        if (beneficiary?.bank_information?.bank_book_photo_url) {
          documents.push({
            id: `bank-book-${id}`,
            type: 'bank_book',
            title: 'Bank Book',
            file_url: beneficiary.bank_information.bank_book_photo_url,
            created_at: beneficiary.created_at,
            isSystemDocument: true
          });
        }

        setBeneficiaryDocuments(documents);
      } catch (err) {
        console.error('Error fetching beneficiary documents:', err);
  ...
  const handleGraduate = async () => {
    ...
    try {
      ...
      // Delete sponsorship records for this beneficiary
      const deleteSponsorshipResponse = await fetch(`http://localhost:5000/api/sponsorships/beneficiary/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      if (!deleteSponsorshipResponse.ok) {
        console.warn('Warning: Failed to delete sponsorship records, but continuing with graduation');
      }
  ...
  const handleTerminate = async () => {
    ...
    try {
      ...
      if (!deleteSponsorshipResponse.ok) {
        console.warn('Warning: Failed to delete sponsorship records, but continuing with termination');
      }
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
              ...
              {beneficiaryDocuments.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>No documents found for this beneficiary.</p>
                </div>
              ) : (
                  ...
                  {beneficiaryDocuments.map((document) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center">
                          {document.type === 'graduation' ? (
                            <GraduationCap className="w-6 h-6 text-green-600 mr-2" />
                          ) : document.type === 'termination' ? (
                            <AlertTriangle className="w-6 h-6 text-red-600 mr-2" />
                          ) : document.type === 'support_letter' ? (
                            <FileText className="w-6 h-6 text-blue-600 mr-2" />
                          ) : document.type === 'bank_book' ? (
                            <CreditCard className="w-6 h-6 text-purple-600 mr-2" />
                          ) : (
                            <FileText className="w-6 h-6 text-blue-600 mr-2" />
                          )}
                          <span className="text-sm font-medium text-gray-900 capitalize">
                            {document.type === 'support_letter' ? 'Support Letter' :
                             document.type === 'bank_book' ? 'Bank Book' :
                             document.type}
                          </span>
                        </div>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          {document.type === 'graduation' ? 'Graduation' : 
                           document.type === 'termination' ? 'Termination' :
                           document.type === 'support_letter' ? 'Support Letter' :
                           document.type === 'bank_book' ? 'Bank Book' :
                           document.type}
                        </span>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
                        ...
                        {document.file_url && (
                          <span className="text-blue-600 hover:text-blue-800 cursor-pointer" 
                                onClick={() => window.open(`http://localhost:5000${document.file_url}`, '_blank')}>
                            View File
                          </span>
                        )}
                      </div>
                      ...
                      {document.file_url && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <p className="text-xs text-gray-500">
                            File: {document.file_url.split('/').pop()}
                          </p>
                        </div>
                      )}
                    </div>
            ...
            <div className="p-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-start">
                  ...
                  <div>
                    <h3 className="font-medium text-blue-800">Confirm Graduation</h3>
