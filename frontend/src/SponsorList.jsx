...
const SponsorList = () => {
  ...
  useEffect(() => {
    ...
    let filteredData = allSponsors.filter(sponsor => {
      ...
      switch (beneficiaryFilter) {
        case "child":
          beneficiaryMatch = (sponsor.beneficiaryCount?.children || 0) > 0 && (sponsor.beneficiaryCount?.elders || 0) === 0;
          break;
        case "elderly":
          beneficiaryMatch = (sponsor.beneficiaryCount?.elders || 0) > 0 && (sponsor.beneficiaryCount?.children || 0) === 0;
          break;
        case "both":
          beneficiaryMatch = (sponsor.beneficiaryCount?.children || 0) > 0 && (sponsor.beneficiaryCount?.elders || 0) > 0;
          break;
  ...
  }, [allSponsors, searchInput, typeFilter, residencyFilter, beneficiaryFilter, currentSortColumn, currentSortDirection]);
  ...
  const getSortIndicator = (columnIndex) =>
    columnIndex === currentSortColumn ? (
      currentSortDirection === "asc" ? <ChevronUp className="w-4 h-4 inline ml-1" /> : <ChevronDown className="w-4 h-4 inline ml-1" />
    ) : null;
  ...
  const getBeneficiaryCountClasses = (children, elders) => {
    if (children > 0 && elders > 0) return "bg-gradient-to-r from-[#ffebee] to-[#fff2e8] text-[#389e0d]";
  ...
  useEffect(() => {
    if (allSponsors.length > 0) {
      console.log('All sponsors beneficiary counts:', allSponsors.map(s => ({
        id: s.id,
        name: s.name,
  ...
  const childOnlySponsors = allSponsors.filter(sponsor => {
    const children = sponsor.beneficiaryCount?.children || 0;
    const elders = sponsor.beneficiaryCount?.elders || 0;
    ...
    <div className="min-h-screen bg-[#f5f7fa] p-4 sm:p-6 lg:p-8 font-inter text-[#1e293b]">
      <div className="container mx-auto bg-[#ffffff] rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.08)] p-4 sm:p-6 lg:p-8 flex flex-col h-[90vh]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#0066cc] bg-gradient-to-br from-[#e0f2ff] to-[#cce5ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${activeCard === "Private" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("type", "Private")}
          >
            <div className="text-3xl font-bold text-[#1e293b]">{privateSponsors}</div>
            <div className="text-sm text-[#64748b]">Private Sponsors</div>
          </div>
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#08979c] bg-gradient-to-br from-[#e6f7ff] to-[#d1f0ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${activeCard === "Organization" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("type", "Organization")}
          >
            <div className="text-3xl font-bold text-[#1e293b]">{organizationSponsors}</div>
            <div className="text-sm text-[#64748b]">Organizations</div>
          </div>
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#0b6bcb] bg-gradient-to-br from-[#e6f4ff] to-[#cce5ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${activeCard === "Local" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("residency", "Local")}
          >
            <div className="text-3xl font-bold text-[#1e293b]">{localSponsors}</div>
            <div className="text-sm text-[#64748b]">Local</div>
          </div>
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#00838f] bg-gradient-to-br from-[#e0f7fa] to-[#b2ebf2] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${activeCard === "Diaspora" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("residency", "Diaspora")}
          >
            <div className="text-3xl font-bold text-[#1e293b]">{diasporaSponsors}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#0066cc] bg-gradient-to-br from-[#e0f2ff] to-[#cce5ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${beneficiaryFilter === "child" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("beneficiary", "child")}
          >
            <div className="text-2xl font-bold text-[#1e293b]">{childOnlySponsors}</div>
            <div className="text-sm text-[#64748b]">Child Beneficiaries Only</div>
          </div>
          
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#08979c] bg-gradient-to-br from-[#e6f7ff] to-[#d1f0ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${beneficiaryFilter === "elderly" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("beneficiary", "elderly")}
          >
            <div className="text-2xl font-bold text-[#1e293b]">{elderlyOnlySponsors}</div>
            <div className="text-sm text-[#64748b]">Elderly Beneficiaries Only</div>
          </div>
          
          <div
            className={`p-4 rounded-lg shadow-[0_3px_8px_rgba(0,0,0,0.05)] border-l-4 border-[#0b6bcb] bg-gradient-to-br from-[#e6f4ff] to-[#cce5ff] cursor-pointer transition-transform duration-200 hover:scale-[1.02] ${beneficiaryFilter === "both" ? "!border-[#032990] !bg-gradient-to-br !from-[#f0f3ff] !to-[#e6eeff]" : ""}`}
            onClick={() => handleCardFilterClick("beneficiary", "both")}
          >
            <div className="text-2xl font-bold text-[#1e293b]">{bothSponsors}</div>
        ...
        <div className="overflow-x-auto flex-1 border border-[#e2e8f0] rounded-lg shadow-[0_2px_5px_rgba(0,0,0,0.05)]">
          <table className="min-w-full divide-y divide-[#e2e8f0]">
            <thead className="bg-[#f0f3ff] sticky top-0">
              <tr>
                {["Sponsor ID","Name","Type","Residency","Phone Number","Beneficiary Count"].map((header,index)=>(
                  <th
                    key={header}
                    className={`px-6 py-3 text-left text-xs font-medium text-[#032990] uppercase tracking-wider cursor-pointer hover:bg-[#e0e8ff] transition-colors duration-200 ${index === 0 ? "rounded-tl-lg" : index === 5 ? "rounded-tr-lg" : ""}`}
            ...
            <tbody className="bg-[#ffffff] divide-y divide-[#e2e8f0]">
              {displayedSponsors.map((sponsor)=>(
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#032990]">{sponsor.id || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#1a1a1a]">{sponsor.name || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getSponsorTypeClasses(sponsor.type)}`}>
                      {sponsor.type || 'N/A'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getResidencyClasses(sponsor.residency)}`}>
                      {sponsor.residency || 'N/A'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-[#64748b]">{sponsor.phone || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getBeneficiaryCountClasses(
                      sponsor.beneficiaryCount?.children || 0, 
                      sponsor.beneficiaryCount?.elders || 0
                    )}`}>
                      {(sponsor.beneficiaryCount?.children || 0) > 0 && (
                        <span className="text-[#c62828]">{`${sponsor.beneficiaryCount.children} ${sponsor.beneficiaryCount.children === 1 ? "child" : "children"}`}</span>
                      )}
                      {(sponsor.beneficiaryCount?.children || 0) > 0 && (sponsor.beneficiaryCount?.elders || 0) > 0 && " & "}
                      {(sponsor.beneficiaryCount?.elders || 0) > 0 && (
                        <span className="text-[#d46b08]">{`${sponsor.beneficiaryCount.elders} ${sponsor.beneficiaryCount.elders === 1 ? "elder" : "elders"}`}</span>
                      )}
                      {(sponsor.beneficiaryCount?.children || 0) === 0 && (sponsor.beneficiaryCount?.elders || 0) === 0 && (
